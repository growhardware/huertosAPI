module.exports = {
  ...
  fn: async function () {
    ...
    const strainIds = user["managing"].map((d) => d.id);
    let strains = await Strain.find({ id: strainIds }).populate("history");

    if (this.req.isSocket) {
      const ids = strains.map((d) => d.id);
      Strain.subscribe(this.req, ids);
    }
